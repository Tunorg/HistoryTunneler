...
class States:
    ...
    # Статы взяты из:
    # http://squarefaction.ru/game/final-fantasy-viii/articles/10480-parametry-geroev-v-final-fantasy-viii
    hp = 0  # Hit Points (НР) - параметр, показывающий количество единиц Жизненной Силы персонажа. Если он снизится до «0», то герой
    ...
    str = 0  # Strength (Str) Физическая Сила персонажа. Напрямую влияет на урон, наносимый противникам в бою с помощью стандартных
             ...
             # «Статусную Атаку».
    vit = 0  # Vitality (Vit) Живучесть персонажа. Даёт в бою защиту от физических ударов, правда, в начале игры её эффективность
    ...
    mag = 0  # Magic (Mag) Магическая Сила персонажа. Напрямую влияет на величину Магического Урона, а также эффективность
    ...
    spr = 0  # Spirit (Spr) Духовная Сила персонажа. Предоставляет в бою защиту от действий магических заклинаний противников.
    ...
    spd = 0  # Speed (Spd) Скорость персонажа. Напрямую влияет на интенсивность действий героя в бою. Чем сильнее развит
    ...
    eva = 0  # Evasion (Eva) Уклонение. С помощью этого параметра персонажи имеют возможность в бою уворачиваться от физических
    ...
    hit = 0  # Hit Percentage (Hit) Точность. Параметр, влияющий на успешность проведения физической Атаки персонажа. При маленьких
...
class Character:
    ...
    def __repr__(self):
        return "{} lvl: {}. {}".format(self.name, self.states.level, self.description)
